...
def check_captcha(captcha_id, captcha_code):
    """ 验证码校验方法 """
    CAPTCHA_CACHE_NAME = 'captcha'
    key = f'captcha:{captcha_id}'
    right_code = cache_verify_service.get_verify_code(key, cache=CAPTCHA_CACHE_NAME)
    cache_verify_service.del_verify_code(key, cache=CAPTCHA_CACHE_NAME)
...
class CaptchaValidateMixin:
    ...
    def validate_captcha(self, attrs):
        ...
        if not check_captcha(captcha_id, captcha_code):
